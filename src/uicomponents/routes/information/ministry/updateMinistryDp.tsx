...
interface IUpdateMinistryDp extends IStyledFC {
...
const UpdateMinistryDPFC: React.FC<IUpdateMinistryDp> = ({className, onLoading, data, update}) => {
    const addSnackBar = useAddSnackBar()
    const [displayPic, setDisplayPic] = React.useState(data.picture);
    const [selectedDisplayPicture, setSelectedDisplayPicture] = React.useState<null | string>(null)
            ...
            <div className="edit-picture-area">
                {
                    displayPic && changePic == false? <>
                        <Avatar src={data.picture} alt={data.ministryName} size="150px" />
                        <div  className="row">
                            <Button disabled={isLoading} icon={<FontAwesomeIcon icon={["fas", "times"]} />} label="Remove picture" onClick={() => {
                                setIsLoading(true)
                                doRequest({
                                    url: `/update-ministry-dp/remove/${data.ministryUID}/${data.picture}`,
                                    method: "PATCH"
                                })
                                .then(response => {
                                    if(response.success) {
                        ...
                        <AvatarPicker onChange={(avatar) => {
                            ...
                            if(avatar) {
                                setIsLoading(true)
                                doRequest({
                                    url: `/update-ministry-dp/update/${data.ministryUID}/${avatar}`,
                                    method: "PATCH"
                                })
                                .then(response => {
                                    if(response.success) {
                        ...
                        {/* <div className="row">
                            <Button icon={<FontAwesomeIcon icon={["fas", "image"]} />} label="Change picture" color="edit" onClick={() => setChangePic(true)} />
                        </div> */}
                    </>
                }
            </div>
...
const UpdateMinistryDP = styled(UpdateMinistryDPFC)`
    display: flex;
    flex: 0 1 100%;
...
export default UpdateMinistryDP;