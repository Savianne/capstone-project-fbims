...
interface IAddOrganizationyMemberSearchComp extends IStyledFC {
    close: () => void,
    organizationUID: string
} 

const AddOrganizationMemberSearchComp: React.FC<IAddOrganizationyMemberSearchComp > = ({close, organizationUID}) => {
...
const SearchResultItem: React.FC<{item: ISearchResult, organizationUID: string, onAddSuccess: () => void}> = ({item, organizationUID, onAddSuccess}) => {
        ...
        <div key={item.memberUID} className="result-item">
            ...
            <div className="btn-area">
                ...
                onClick={() => {
                    setIsLoading(true);
                    doRequest<null>({
                        url: "/add-member-to-organization",
                        method: "POST",
                        data: {
                            organizationUID: organizationUID,
                    ...
                    .then(response => {
                        if(response.success) {
                            ...
                            addSnackBar("Successfully added new member to organization", "success", 5);
                        } else throw response
                    })
                    .catch(err => {
                        setIsLoading(false);
                        setIsError(false);
                        addSnackBar("Failed to add new member to organization", "error", 5);
                    })
                }}/>
            </div>
        </div>
...
export default AddOrganizationMemberSearchComp; 